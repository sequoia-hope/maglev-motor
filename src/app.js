...
const PRESETS = {
  ...
  pcb: {
    ...
    blurb: 'The larger of the two PCB stages, same recipe as the small one: Carl Bugeja\'s 12-layer coil stitched with plated through-holes only (no blind/buried vias, so a fab will actually press it), 1 oz (35 µm) copper at its 0.103 mm minimum trace, 13 turns per layer on a 1.57 mm board — wound deep to the centre, corners rounded, and every layer\'s spiral walked a fraction of a turn further round so its end fans out and its crossover via drops straight off the winding (a short radial stub, never a tab that rings around). Every plated through-hole is checked: none contacts a layer it must not (validatePcb), at any fill. Copper weight is the wall — Bugeja\'s fab rule (4 mil per ounce) means you cannot have both many turns and low current density. Over a 72 mm 2-D Halbach platen at λ = 24 mm it flies everywhere, no dead spots: 17.6× lift centred and 17.4× at the workspace corners, at a 2.0 mm gap over a 1.79 mm floor, off 324 spirals grouped into 36 amplifiers, 11.8 W hover, 24 K board rise. Still heat-first: the thin 1 oz copper carries the hover current at 79 A/mm². The magnets are catalogue stock — 6×6×3 mm N52 blocks (Supreme), 25 axial + 60 in-plane (the in-plane block is the same part magnetised through a 6 mm edge), 85 magnets in 121 pockets, 66.0 mm across on a 72 mm platen. For a lighter, cooler board see the small PCB stage.',
    cfg: {
  ...
  amzhex: {
    ...
    blurb: 'The Amazing Magnets build of the driver-per-coil honeycomb: every magnet is a 1/4″ × 1/4″ × 1/8″ N52 brick, and BOTH magnetisations are catalogue parts — Q125B-N52, magnetised through the 1/8″ thickness, is the axial block; P250AB-N52, magnetised through a 1/4″ width, is the in-plane block. No jig-turning (a turned brick would stand 1/4″ tall): the flux-steering orientation is its own SKU, the platen carries half a cube array\'s magnet mass, and the whole order is $20.22 — 9 of one part and 24 of the other, and P250AB ships in dozens, so 24 is exactly two. λ = 25.4 mm because that is 4 × 1/4″, so the platen is a symmetric 7×7 lattice, 44.45 mm across on a 44.45 mm platen with zero border wasted: 33 magnets in 49 pockets, the pattern nulls left empty. Over it, the pcbhexdrv recipe rescaled to the inch pitch: a honeycomb of 168 coils on a 12-layer, plated-through-hole, 1 oz board whose bottom copper is surrendered to the electronics — each coil winds 144 turns (12 per layer) of 0.103 mm trace, a 10.0 Ω load whose current limit IS the bus: 0.9 A wants exactly 9 V, so the rail and the ceiling are the same number — sub-amp bridge silicon (a six-pin SOT-23-6 H-bridge), no per-coil current sensing (commanded duty × bus ÷ calibrated R is the current). The stack is FOURTEEN layers: twelve of winding plus two surrendered to the electronics, added rather than taken out of the coil. Both halves of that matter. The electronics layers have to come in a PAIR because a spiral alternates which end it finishes on — an odd winding count strands the last lead in the centre hole, where the tab carrying it out crosses every turn of its own layer and shorts that layer on all 168 coils, invisibly, because the tab and the winding are the same net. And they have to be ADDED because one electronics layer cannot carry the interconnect and taking two out of twelve costs a fifth of the winding for nothing. The price is a 14-layer board, past where PCB pricing turns steep, and a 0.23 mm via drill: 1.84 mm of board through a 0.2 mm hole is 9:1 and the fab plates 8:1. The winding sits at 80% fill rather than 84% for a related reason — the gutter has to hold a via the fab will really drill (0.5 mm land, a 0.135 mm ring; JLCPCB\'s floor is 0.25 mm on 0.15 mm), and 84% leaves 0.59 mm of gutter where 0.79 mm is needed. That costs no winding at all: turns-per-layer is floor()-quantised and comes out 12 at either fill. At that honest ceiling it lifts 3.6× centred and 2.3× at the corners of a ±16 mm workspace — and the honeycomb\'s yaw isotropy holds, 2.3× worst-case with the platen turned 45° — at a 1.5 mm gap over a 1.36 mm floor. The busiest hover coil draws 0.25 A at 2.5 V; hover is 4.6 W centred and ≈5.6 W worst-case (~22 K rise), and like every 1 oz honeycomb it is heat-first, 70 A/mm² in the thin copper at hover. The board outline follows the honeycomb cell boundary (~106 mm, 12 rows — even, so full boards interlock with copies of themselves and continue the lattice; seam 0.41 mm). Pick this over amz316 when you want the independent-driver electronics and a lighter platen; pick it over pcbhexdrv when the supplier has to be one US catalogue.',
    cfg: {
      translator: { arrayType: 'halbach2d', layout: 'single',
      ...
      // 0.80 fill, not 0.84: the gutter has to hold a via the fab will really
      // drill. A 0.5 mm land on a 0.2 mm hole (0.15 mm ring) needs 0.09 mm off
      // the winding copper and 0.2 mm off the board edge -- 0.79 mm of gutter,
      // and 0.84 fill leaves 0.59. 0.80 leaves 0.80, and costs NOTHING in
      // winding: turns-per-layer is floor()-quantised and comes out 12 (132
      // turns) at either fill. See gutterFits() in kicad.js.
      // TWO electronics layers, not one, for two independent reasons.
      // (1) The winding-layer count must be EVEN. A spiral alternates: layer 0
      //     starts outside, layer 1 starts inside, ... so with an odd count the
      //     last layer ENDS in the centre hole and the tab carrying that lead
      //     out to its I/O pad crosses every turn of its own layer -- a dead
      //     short of the whole layer, on all 168 coils, which no PCB DRC can
      //     see because the tab and the winding are the same net. validatePcb
      //     checks it now; 11 winding layers fail it and 10 pass.
      // (2) One electronics layer cannot carry the interconnect. Routed on
      //     B.Cu alone the board tops out around half its connections however
      //     many passes the autorouter is given -- there is nowhere to cross.
      // And FOURTEEN layers, not twelve: the two electronics layers are ADDED
      // rather than taken out of the winding, so the coils keep all twelve and
      // 144 turns -- more than the 132 the broken odd-layer build claimed, and
      // 20% more than surrendering two of twelve. It costs a 14-layer stack,
...
function renderBuild() {
  ...
  if (isPcbCoil(app.cfg.stator.coilType)) {
    ...
    pcbCard.innerHTML = `<h4 style="font-size:12px;margin:0 0 8px;color:var(--muted);text-transform:uppercase;letter-spacing:.05em">PCB stator export</h4>
      ...
      <button id="btnFwMap" class="ghost">Firmware map (JSON)</button>
      <p style="font-size:12px;color:var(--muted);margin:8px 0 0">The <b>3×3 tile</b> is a ${(3 * app.cfg.stator.coilPitch * 1000).toFixed(0)} mm, 9-coil unit — small enough to fab cheaply and panelise, and it opens fast. Wire the edge coils' I/O pads to their neighbours or a backplane.</p>
      ${(() => {
        const t = s.tile;
        if (!t?.available) return '';
        const msg = t.tileable
          ? `<b>Self-tileable:</b> the outline follows the coil-cell boundary, so full boards interlock and continue the lattice — period ${t.periodXmm.toFixed(0)} × ${t.periodYmm.toFixed(1)} mm, seam copper-to-copper ${t.seamGapMm.toFixed(2)} mm. Abut copies to grow the stator; the platen never sees the seam.`
          : !t.latticeY
            ? `<b>Not self-tileable vertically:</b> the honeycomb's row-offset alternation breaks at the seam with ${t.nRows} (odd) rows — nudge the stator size to an even row count.`
            : `<b>Not self-tileable:</b> rim copper reaches ${t.seamGapMm.toFixed(2)} mm at the seam (this grid's I/O pads straddle the cell boundary at this fill) — abutted boards would collide; lower the fill, or grow this board instead of tiling it.`;
        return `<p style="font-size:12px;color:${t.tileable ? 'var(--muted)' : 'var(--warn)'};margin:8px 0 0">${msg}</p>`;
      })()}
...
function renderFitCheck() {
  ...
  const ap = r.asPlaced;
  const apTxt = ap?.available
    ? (ap.fracEstimable >= 0.999
      ? `Re-swept at the <b>as-placed</b> spots: <span class="ok">all ${ap.nPose} poses estimable</span>, worstObs ${ap.worstObs.toFixed(3)} — the nudges cost nothing.`
      : `Re-swept at the <b>as-placed</b> spots: <span class="bad">only ${(ap.fracEstimable * 100).toFixed(0)}% of poses estimable</span> — the nudges broke the grid; refine placement.`)
    : '';
